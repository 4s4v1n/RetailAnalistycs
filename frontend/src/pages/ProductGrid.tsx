...
import ProductGridService from "../services/ProductGridService";

const ProductGrid: FC = () => {

    const queryClient = useQueryClient()

    const name = "product_grid"

    const { data } = useQuery([{ name }], () => ProductGridService.get(),)

    const { mutate: mutationAdd } = useMutation({
        mutationFn: ProductGridService.post,
        onSuccess: () => {
    ...
    const { mutate: mutationUpdate } = useMutation({
        mutationFn: ProductGridService.patch,
        onSuccess: () => {
    ...
    const { mutate: mutationDelete } = useMutation({
        mutationFn: ProductGridService.delete,
        onSuccess: () => {
    ...
    const { mutate: mutateExport } = useMutation({
        mutationFn: ProductGridService.export,
        onSuccess: (data) => {
    ...
    const { mutate: mutateImport } = useMutation({
        mutationFn: ProductGridService.import,
        onSuccess: () => {
